...
var Letter = P(Variable, function(_, super_) {
  ...
  _.createLeftOf = function(cursor) {
    ...
    var autoOps = cursor.options.autoOperatorNames;
    var addParen = false;
    ...
    if (maxLength > 0) {
      ...
      // check for an autocommand, making sure to only match entire strings so
      // we don't have to deal with issues with commands like epsilon, which
      // end up bugged because there's a psi in it
      ...
      // Also check for autoOperatorNames that need to have a parenthesis added.
      // If str isn't an exact match, then start slicing off letters
      // starting at the start of the string until we either find an autoOp,
      // or we're out of characters. While we don't want a psi appearing inside
      // of epsilon, we do want parens added when a user enters something like
      // xsin(x) as it won't impede them from entering the intended response
      while (str.length) {
        if (autoOps.hasOwnProperty(str) && !(cursor[R] instanceof Bracket)) {
          // Check for the three autoOps that aren't actually functions
          if (str !== 'NOSOLUTION' && str !== 'UNDEFINED' && str !== 'DNE') {
            addParen = true;
          }
        }
        str = str.slice(1);
...
(function() {
  ...
  var mostOps = ('arg deg det dim gcd hom inf ker lg lim ln log max min sup'
...
var Vector = P(Symbol, function(_, super_) {
  _.init = function(ch, html) {
    super_.init.call(this, ch, '<span class="mq-unit-vector">'+(html || ch)+'</span>');
  };
});
