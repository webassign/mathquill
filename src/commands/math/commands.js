...
var SupSub = P(MathCommand, function(_, super_) {
  ...
  _.createLeftOf = function(cursor) {
    ...
    // If we're immediately to the right of a fraction, wrap the fraction in parens
    // so that a sub can't be confused as part of the denominator and a sup can't be
    // confused as part of the numerator
    if (cursor[L] instanceof LiveFraction) {
      // We need to get access to the API so that we can get to typedText and keystroke.
      // However, its only available to the root node, so keep iterating through the
      // cursor's parents until we reach it
      var thisParent = cursor;
      while (thisParent.parent) {
        thisParent = thisParent.parent;
      }
      var api = thisParent.controller.API;
      // Select the fraction to the left, wrap it in parens, then tab back to where we started
      api.keystroke("Shift-Left");
      api.typedText("(");
      api.keystroke("Tab");
    }
...
CharCmds['/'] = P(Fraction, function(_, super_) {
  _.createLeftOf = function(cursor) {
    ...
    // Check to see if we're in an exponent
    var nodeParent = cursor.parent;
    var isExponent = false;
    while (nodeParent) {
      if (nodeParent.jQ.hasClass("mq-sup")) {
        isExponent = true;
        break;
      }
    ...
    // If we're in an exponent, grab the API object
    // from the root node, then use it to wrap our
    // fraction in parens
    if (isExponent) {
      // Grab the API object
      var thisParent = cursor.parent;
      while (thisParent.parent) {
        thisParent = thisParent.parent;
      }
      var api = thisParent.controller.API;
...
var Hat = LatexCmds.hat = P(MathCommand, function(_, super_) {
  ...
  _.htmlTemplate =
      '<span class="mq-non-leaf">'
    +   '<span class="mq-vector-hat-prefix">&circ;</span>'
    +   '<span class="mq-vector-hat-stem">&0</span>'
    + '</span>'
  ;
  _.textTemplate = ['hat(', ')'];
});

var Bold = LatexCmds.bold = P(MathCommand, function(_, super_) {
  _.ctrlSeq = '\\bold';
  _.htmlTemplate =
      '<b class="mq-text-mode">&0</b>';
  ;
  _.textTemplate = ['bold(', ')'];
});

var ExponentialFunction = LatexCmds.exp = P(MathCommand, function(_, super_) {
  _.ctrlSeq = '\\exp';
  _.htmlTemplate =
      '<var>e</var>'
    + '<span class="mq-supsub mq-non-leaf mq-sup-only">'
    ...
    + '</span>'
  ;
  _.textTemplate = ['exponentialFunction(', ')'];
});
...
LatexCmds.lparen = bind(Bracket, L, '(', ')', '(', ')');
LatexCmds.lbracket = bind(Bracket, L, '[', ']', '[', ']');
LatexCmds.lopenint = bind(Bracket, L, '(', ']', '(', ']');
LatexCmds.ropenint = bind(Bracket, L, '[', ')', '[', ')');
// capital B so we don't clobber the lbrace delimiter
LatexCmds.lBrace = bind(Bracket, L, '{', '}', '{', '}');
